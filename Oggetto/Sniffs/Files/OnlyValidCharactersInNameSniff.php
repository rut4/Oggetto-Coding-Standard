...
class Oggetto_Sniffs_Files_OnlyValidCharactersInNameSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = 0; $i < strlen($fileName); $i++) { 
            ...
            if (!(in_array(ord($char) - ord("0"), $nums) || in_array($char, $lowerLetters) || in_array($char, $upperLetters) || $char == "-" || $char == "_")) {
                $phpcsFile->addError("Character '%s' should not be used in file name", $stackPtr, "Found", $fileName[$i]);
            }
        }
    }
}
